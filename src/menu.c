...
void		menu_init(Menu* d)
{
  ...
  d->srf[SRF_MENU_KEYBOARD] = SDLazy_SurfaceLoad("img/menu_keyboard.png", SDLazy_Rect(0, 0, 128, 36));
  d->srf[SRF_MENU_OPEN]     = SDLazy_SurfaceLoad("img/menu_open.png", SDLazy_Rect(0, 0, 32, 32));
...
void		menu_blit(Menu* d)
{
  ...
  double	h2 = SDLazy_GetHeight(0) / 2.0;

  SDLazy_SetScaleY(d->srf[SRF_MENU_BG], h2);
